...
class Robot:
    ...
    def attack(self, dinosaur):
        ...
        self.active_weapons.append(Weapon("Circular Saw Arm", 45))
        self.active_weapons.append(Weapon("Arm Cannon", 35))
        self.active_weapons.append(Weapon("Core Blast", 75))
        select_weapon = random.choice(self.active_weapons)

        if select_weapon == self.active_weapons[0]:
            print(f"{self.name} attacked {dinosaur.name} with {select_weapon},\n dealing {self.active_weapons[0].attack_power}!")
